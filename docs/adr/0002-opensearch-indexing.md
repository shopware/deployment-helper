...
- Run `es:index --no-queue` when `OPENSEARCH_URL` is set and `SHOPWARE_ES_INDEXING_ENABLED` is set to 1.
- Run `es:admin:index --no-queue` when `ADMIN_OPENSEARCH_URL` is set and `SHOPWARE_ES_INDEXING_ENABLED` is set to 1.
