...
4. `post-extension-on-project-install` hook
6. `post-install` hook
...
3. Maintenance enabled: persists per-storefront state snapshot (`system_config` key `deployment.maintenanceMode`), enables maintenance, clears cache (failure leaves maintenance on—no finally; the snapshot survives for the retry)
...
7. `post-update` hook
8. Maintenance disabled: restores per-storefront prior states from the snapshot, deletes the snapshot, clears cache
...
- **Hooks:** `pre`, `post`, `pre-install`, `post-extension-on-project-install`, `post-install`, `pre-update`, `post-update` - inject shell commands at deploy moments. Untyped escape hatch.
...
- **Maintenance mode:** optional, update-only. No safety guard on prod. Can fail and leave enabled; the persisted snapshot lets a retry restore the original per-channel state. Fresh installs do not use it.
